...
| 7  | `k`                   | int     | Held-out length used for prediction. By construction `k = max(20, total_n / 4)`, fixed in advance. |
...
An earlier draft of the schema included
...
wastes storage and de-normalises the schema. The schema instead keeps
...
```
run_id, csv_path, csv_schema_columns, omnis_sha,
